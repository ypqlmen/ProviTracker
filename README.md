...
Se `docs/power_automate_salgsregistrering.md`, `docs/microsoft_oauth_power_automate_setup.md` og `scripts/excel_online_sales_registration.ts` for den konkrete flow-ops?tning.
...
Appen gemmer data i brugerens lokale AppData, s? almindelige brugere ikke beh?ver administratoradgang:
...
WinSparkle l?ser appcast fra:

`https://raw.githubusercontent.com/ypqlmen/ProviTracker/main/appcast.xml`

GitHub Release-tagget til auto-update er `autoupdate`, og asset-navnet for version 1.3.23 er:

`ProviBeregnerSetup-1.3.23.exe`
...
Scriptet uploader installeren til release-tagget og opdaterer `appcast.xml` p? `main`.
